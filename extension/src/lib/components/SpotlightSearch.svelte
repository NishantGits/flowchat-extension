<script lang="ts">
  ...
  const resetConfig = () => {
    ...
    try {
      ...
      localStorage.setItem("url", "https://flowchats.org");
  ...
  const initHandler = async (e) => {
    ...
    console.log("Saving config:", { key: key ? "***" : "", model, url });

    // Helper function to set storage using the bridge
    const setStorage = async (key, model, url) => {
      try {
        if (typeof window !== 'undefined' && window.flowchatStorage) {
          const result = await window.flowchatStorage.set({ url: "https://flowchats.org", key: key, model: model });
          return result;
        } else if (typeof chrome !== 'undefined' && typeof chrome.storage !== 'undefined') {
          await chrome.storage.local.set({ url: "https://flowchats.org", key: key, model: model });
          return true;
        } else {
    ...
    try {
      await setStorage(key, model, url);
      console.log("Value is set");
    } catch (error) {
      console.log(error);
    }

    // Only hide config if we have both API key and model
    if (key && model) {
      showConfig = false;
    }
  };
  ...
  onMount(async () => {
    ...
    // Helper function to get storage using the bridge
    const getStorage = async () => {
      try {
        if (typeof window !== 'undefined' && window.flowchatStorage) {
          const result = await window.flowchatStorage.get();
          return result;
        } else if (typeof chrome !== 'undefined' && typeof chrome.storage !== 'undefined') {
          ...
          return {
            ...
            url: localStorage.getItem("url") || "https://flowchats.org"
          };
        }
      } catch (error) {
    ...
    try {
      _storageCache = await getStorage();
    } catch (error) {
      console.log(error);
    }
    ...
    if (_storageCache) {
      ...
      console.log("Loaded from storage:", { key: key ? "***" : "", model, url });
      
      // Try to get models if we have an API key
      if (_storageCache.key) {
        try {
          models = await getModels(_storageCache.key, url);
          ...
          if (!model && models && models.length > 0) {
            ...
            try {
              await chrome.storage.local.set({ url: "https://flowchats.org", key: key, model: model });
              console.log("Auto-selected model saved:", model);
            } catch (error) {
              console.log("Error saving auto-selected model:", error);
            }
          }
          
          // Hide config if we have API key (models can be loaded later)
          showConfig = false;
        } catch (error) {
          console.log("Error loading models:", error);
          // Don't reset config - keep API key and try again later
          // Only hide config if we have both API key and a selected model
          if (model) {
            showConfig = false;
          }
        }
        ...
        >
          ...
          >
            <div class="tlwd-flex tlwd-items-center tlwd-gap-2 tlwd-w-full">
              ...
              <input
                id="flowchat-url-input"
                placeholder="FlowChat URL"
            ...
            >
              ...
              <button
                ...
                type="button"
                on:click={() => showApiInfo = !showApiInfo}
            ...
            {#if showApiInfo}
              <div class="tlwd-mt-2 tlwd-p-3 tlwd-bg-gray-800 tlwd-rounded-lg tlwd-border tlwd-border-gray-700">
                <div class="tlwd-text-sm tlwd-text-gray-300 tlwd-font-medium mb-2">How to get a FlowChat API Key:</div>
                <div class="tlwd-text-xs tlwd-text-gray-400 leading-relaxed">
                  Go to FlowChat → Profile → Settings → Account → API Keys and create / copy the key.
                </div>
              </div>
            {/if}
        ...
        >
          ...
          >
            <div class="tlwd-flex tlwd-items-center tlwd-gap-2 tlwd-w-full">
              ...
              <input
                id="flowchat-search-input"
                placeholder="Ask FlowChat..."
                class="tlwd-p-0 tlwd-m-0 tlwd-text-xl tlwd-w-full tlwd-font-medium tlwd-bg-transparent tlwd-border-none placeholder:tlwd-text-gray-500 tlwd-text-neutral-100 tlwd-outline-none"
